...
const students = [
  ...
  function santasNaughtyList(persons, badHabit) {
    ...
    for (const eachPerson of persons) {
        // console.log(eachPerson["habits"])
        for (const eachHabit of eachPerson["habits"]) {
            if (eachHabit === badHabit) {
                result.push(eachPerson["firstName"]+ " " +eachPerson["lastName"])
                break;
            }
        }      
    }
  ...
  function santasNaughtyListFunctional(persons, badHabit) {
    return persons
                ...
                .map((filteredPerson)=>`${filteredPerson.firstName} ${filteredPerson.lastName}`)
  }
