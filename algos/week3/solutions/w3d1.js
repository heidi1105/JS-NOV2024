...
function entries(obj) {
    ...
    for (const key in obj) {
        if (obj.hasOwnProperty(key)) { // Object.hasOwn(obj, key)
...
function insert(tableName, columnValuePairs) {
    ...
    for (const key in columnValuePairs) { 
        if (columnValuePairs.hasOwnProperty(key)) {
            keyList.push(key)
            valList.push(columnValuePairs[key])
        }
    }
    const resultString = `INSERT INTO ${tableName} (${keyList.join(", ")}) VALUES (${valList.join(", ")});`
    return resultString;
}   
